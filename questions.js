...
const uniqueKey = 'Introduction to propotion'
  ...
  if (Array.isArray(randomQuestion.question)) {
    ...
    randomQuestion.question.forEach(part => {
      if (typeof part === 'string') {
        ...
        let numeratorElement = null;
        let denominatorElement = null;

        // Handle whole number
        if (whole !== undefined && whole !== '') {
          if (whole === '______') {
            // Create input for whole number
            wholeElement = createInputElement(randomQuestion.input[inputIndex]);
            wholeElement.style.width = '60px';
            wholeElement.style.maxWidth = '60px';
            wholeElement.style.margin = '0 5px';
            inputIndex++;
          } else {
            fractionHTML += `<span class="whole"><strong>${whole}</strong></span>`;
          }
        } else {
          fractionHTML += '<span class="whole">&nbsp;</span>';
        }

        // Build fraction structure
        fractionHTML += '<span class="fraction">';
        
        // Handle numerator
        if (numerator === '______') {
          numeratorElement = createInputElement(randomQuestion.input[inputIndex]);
          numeratorElement.style.width = '60px';
          numeratorElement.style.maxWidth = '60px';
          numeratorElement.style.margin = '2px';
          fractionHTML += '<span class="numerator fraction-input-placeholder"></span>';
          inputIndex++;
        } else {
          fractionHTML += `<span class="numerator"><strong>${numerator}</strong></span>`;
        }

        // Handle denominator
        if (denominator === '______') {
          denominatorElement = createInputElement(randomQuestion.input[inputIndex]);
          denominatorElement.style.width = '60px';
          denominatorElement.style.maxWidth = '60px';
          denominatorElement.style.margin = '2px';
          fractionHTML += '<span class="denominator fraction-input-placeholder"></span>';
          inputIndex++;
        } else {
          fractionHTML += `<span class="denominator"><strong>${denominator}</strong></span>`;
        }

        fractionHTML += '</span>';
        
        // Set the HTML structure
        span.innerHTML = fractionHTML;
        
        // Insert input elements for whole, numerator, or denominator
        if (wholeElement) {
          let wholeSpan = span.querySelector('.whole');
          if (wholeSpan) {
            wholeSpan.appendChild(wholeElement);
          } else {
            // If no whole span exists, insert before the fraction
            span.insertBefore(wholeElement, span.firstChild);
          }
        }
        
        if (numeratorElement) {
          let numeratorSpan = span.querySelector('.numerator.fraction-input-placeholder');
          if (numeratorSpan) {
            numeratorSpan.appendChild(numeratorElement);
            numeratorSpan.classList.remove('fraction-input-placeholder');
          }
        }
        
        if (denominatorElement) {
          let denominatorSpan = span.querySelector('.denominator.fraction-input-placeholder');
          if (denominatorSpan) {
            denominatorSpan.appendChild(denominatorElement);
            denominatorSpan.classList.remove('fraction-input-placeholder');
          }
        }
